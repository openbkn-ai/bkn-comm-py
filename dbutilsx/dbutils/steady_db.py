...
def connect(
    ...
    **kwargs,
    ...
    return SteadyDBConnection(
        creator, maxusage, setsession, failures, ping, closeable, *args, **kwargs
    )
...
class SteadyDBConnection:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if (
            ...
            and not isinstance(failures, tuple)
    ...
    def dbapi(self):
        ...
        if self._dbapi is None:
            raise AttributeError(
                "Could not determine DB-API 2 module (please set creator.dbapi)."
            )
        return self._dbapi
    ...
    def _cursor(self, *args, **kwargs):
        ...
        try:
            ...
            if self._maxusage and self._usage >= self._maxusage and not transaction:
                raise self._failure
...
class SteadyDBCursor:
    ...
    def _get_tough_method(self, name):
        ...
        def tough_method(*args, **kwargs):
            ...
            try:
                # check whether the connection has been used too often
                if con._maxusage and con._usage >= con._maxusage and not transaction:
                    raise con._failure
                if execute:
                    self._setsizes()
