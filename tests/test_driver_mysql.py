...
def test_compatibility():
    ...
    assert hasattr(rdsdriver, "Binary"), "缺少Binary构造函数"
    ...
    # 测试Binary构造函数
    binary_obj = rdsdriver.Binary(b"test data")
    print(f"  ✅ Binary构造函数: {binary_obj}")
...
def test_conn():
    ...
    assert hasattr(cursor, "setoutputsizes")
    cursor.close()
    conn.close()
...
def test_init():
    ...
    cursor = conn.cursor()
    rslt = cursor.execute(f"CREATE DATABASE IF NOT EXISTS {DB_CONFIG['DB']}")
    cursor.close()
    conn.close()
